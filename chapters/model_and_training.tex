...
Several operations were performed during preprocessing for each file separately:

\begin{enumerate}
  \item All comments were removed from the file, because they irrelevant for error detection and increase the sequence length.
  \item All consecutive whitespaces and line breaks were replaced by one single whitespace. Again, this was done to reduce sequence length because Java is a whitespace insensitive programming language, i.e. a Java program is still valid (albeit hard to understand) if it is put on a single line.
