...
To train the model, a big dataset of erroneous code examples produced by real programmers and their respective corrections would be ideal. This would assure a large variety of errors and real life examples. However such a dataset does not exist yet in part because the correction of erroneous code is a long and tedious work.

That's why the errors have to be self introduced to a dataset of correct code. This task is no trivial one and several new difficulties occur. For one, the more sophisticated an error is the harder it is to introduce. Another issue is the artificiality of the errors. One runs the risk of the model picking up on the error generation patterns and thus performing poorly on non-artificial examples. These problems are further discussed in subsection \ref{corruption}.
...
\subsection{Corruptions and their Limitations}
\label{corruption}
...
Of the possible corruptions, two produced syntax errors, two semantic errors and one logic errors. The syntax error consisted of removing a bracket or a semicolon, for the semantic errors a variable was misspelled or the return type of the source changed and the logic errors were produced by switching the order of statement lines. Examples and implementation details of the different corruptions can be found in table \ref{corruption_table}.

Of the five possible corruptions, the syntax errors are the easiest to introduce and come relatively close to reality. They are typical errors that a novice programmer would produce. The other three corruptions produce more sophisticated errors but each one has some limits.

The misspelling of a random variable works generally well. As a convention, the declaration of the variable is considered the "ground truth" and thus only occurrences of the variable after its declaration are misspelled. However there are some cases where the corruption doesn't work as intended. Consider the following code snippet:
...
Lets assume that \texttt{seed} is to be corrupted. One possible misspelling would be to add a random character. If per chance an 's' is added to the end of the occurrence of \texttt{seed} in the third line, it produces another valid variable, namely \texttt{seeds}. Of course the error could still be detected but it is now a logic error and not a semantic one and the model doesn't learn to correct this specific type of error with the other corruptions. It is also possible, that the corruption switches two adjacent characters in \texttt{seed}. If the second and third character are selected it would have no effect and therefore no error would be added. However these two scenarios are very rare and therefore shouldn't impact the ability of the model to learn to find misspelled variables and correct them.

The changing of the return type of the method can be generated consistently. However it imposes some unique problems to the network. One way is pretty simple. If the return type of the method is changed from \texttt{void} to something different, the model needs only check if a return statement is present and if not, the return type should be changed to \texttt{void}. The other way is more complicated. Again the model can determine if the return type \texttt{void} is correct by looking if a return statement is present. However if it determines that it is incorrect the model still needs to derive the correct return type from the given context which is not always possible. For example if the following source is given:
...
\noindent the correct return type is not identifiable because \texttt{this.value} is not defined in the context of this method. It could be any numeric type. This is the tradeoff of only looking at method declarations. However even with the full file context the return type could still be defined in another file. Despite this limitation the corruption can still be used to test the limits of what the model can learn.

Lastly logic errors are the most challenging ones to introduce because they require some form of understanding of the source. To keep the errors as realistic as possible, it was decided to just switch the order of two adjacent lines in the method. However this corruption is not guaranteed to produce a logic error. To increase the probability of an error, some restrictions were put into place. Firstly, only variable declarations, assignments or method invocations are considered and secondly only lines of different types can be switched. This increases the probability of a logic error, but it doesn't guarantee it. Consider the following example:
...
Here the only lines that can be switched are line 3 and 4 but no logic error is produced in doing so. However to guarantee the production of a logic error a deeper understanding of the code would be necessary which is not possible if the errors are produced artificially. Still the corruption is (like the misspelling of variables discussed above) "good enough", meaning it does its purpose most of the time. 
